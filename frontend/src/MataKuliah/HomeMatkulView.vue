                  ...
                  <div class="card-body">
                    ...
                    <div class="table-responsive mt-2">
                      <table id="datatable" class="table table-hover table-bordered display">
                        ...
                        <tbody>
                          <tr v-for="(post, index) in matakuliahs" :key="post.id">
...
export default {
  mounted() {
    axios.get('http://localhost:8080/api/matakuliah').then((response) => {
      this.matakuliahs = response.data.data;
    ...
    PostDelete(id, index) {
      this.$swal({
        ...
        if (result.isConfirmed) {
          // <-- if confirmed
          axios
            .delete(`http://localhost:8080/api/matakuliah/${id}`)
            .then((response) => {
              this.$swal('Deleted!', 'Your file has been deleted.', 'success');
              this.matakuliahs.splice(index, 1);
