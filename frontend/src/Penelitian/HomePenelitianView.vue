                  ...
                  <div class="card-body">
                    ...
                    <div class="table-responsive mt-2">
                      <table id="datatable" class="table table-hover table-bordered display">
                        ...
                        <tbody>
                          <tr v-for="(post, index) in tbl_penelitian" :key="post.id">
                            <td>{{ post.judul }}</td>
                            <td>{{ post.tahun }}</td>
                            <td>{{ post.sumber_dana_pt_mandiri }}</td>
                            <td>{{ post.dana_pt_mandiri }}</td>
                            <td>{{ post.sumber_dalam_negeri }}</td>
                            <td>{{ post.dana_dalam_negeri }}</td>
                            <td>{{ post.sumber_luar_negeri }}</td>
                            <td>{{ post.dana_dalam_negeri }}</td>
...
export default {
  ...
  mounted() {
    axios.get("http://localhost:8080/api/penelitian").then((response) => {
      this.tbl_penelitian = response.data.data;
  ...
  confirmButtonText: 'Yes, delete it!'
    ...
    if (result.isConfirmed) { // <-- if confirmed
        axios
        .delete(`http://localhost:8080/api/penelitian/${id}`)
        .then((response) => {
          this.$swal(
          'Deleted!',
